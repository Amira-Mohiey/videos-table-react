...
export default class VideoTable extends Component {
  ...
  componentDidMount() {
    ...
    this.setState({ videos, filteredVideos: videos })
  }

  filter = name => event => {
    var filteredVideos = this.state.videos.filter(video => {
      return video[name].toLowerCase().includes(event.target.value.toLowerCase())
    });
  ...
  handleSortClick = (name, order) => event => {
    switch (name) {
      case "views":
        this.state.viewsOrder == 'asc' ? this.setState({ viewsOrder: "desc" }) : this.setState({ viewsOrder: "asc" })
        break;
      case "shares":
        this.state.shareOrder == 'asc' ? this.setState({ shareOrder: "desc" }) : this.setState({ shareOrder: "asc" })
        break;
      case "likes":
        this.state.likesOrder == 'asc' ? this.setState({ likesOrder: "desc" }) : this.setState({ likesOrder: "asc" })
        break;
    }
  ...
  renderTable() {
    return (
      <Paper className="table"  >
        ...
        {this.state.filteredVideos && <Table className="table"  >
          <TableHead>
            <TableRow>
              ...
              <TableCell >
                <TableSortLabel
                  ...
                  onClick={this.handleSortClick("views", this.state.viewsOrder)}
                >
                  Views
                  </TableSortLabel>
              </TableCell>
              <TableCell >
                <TableSortLabel
                  ...
                  onClick={this.handleSortClick("likes", this.state.likesOrder)}
                >
                  likes
                  </TableSortLabel>
              </TableCell>
              <TableCell >
                <TableSortLabel
                  ...
                  onClick={this.handleSortClick("shares", this.state.shareOrder)}
                >
                  Shares
                </TableSortLabel>
              </TableCell>
        ...
        { this.state.filteredVideos && !this.state.filteredVideos.length && <p className="result" >No result found</p>}
      </Paper>
    )
  }
  render() {
